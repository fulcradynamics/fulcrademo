...
def filter_and_rank_locations(data, location_type=None, top_n=50):
    ...
    filtered_locations = []
    for loc in data:
        loc_details = loc["location_details"]
        if loc_details:
            if (
                location_type is None
                or loc_details["components"].get("_type") in location_type
            ):
                filtered_locations.append(loc["address"])
...
def get_user_name(dataset):
    return (
        dataset["fulcra_user_name"]
